    ...
    async start(reusePort: boolean = false) {
        try {
            ...
            (this.mcpServer as any).registerTool(
                "ask_user",
                {
                    description: "SUBAGENT RESTRICTION: Subagents (via runSubagent) MUST NOT call this tool. MODES — pick exactly one: (A) Single question + choices → pass 'question' and 'choices' params. Shows clickable buttons above the text input. User can click a button OR type custom text. Example: { question: 'Which DB?', choices: [{label:'1. PostgreSQL',value:'postgresql'},{label:'2. MySQL',value:'mysql'},{label:'3. SQLite',value:'sqlite'}] }. (B) Single open-ended question → pass 'question' param only (no choices, no questions array). Uses the default text input — do NOT add your own text field. Example: { question: 'What would you like to do next?' }. (C) Multi-question form → pass 'questions' array (2-4 items). NEVER for a single question. Each item needs 'header' (≤12 chars) and 'question'. Add 'options' array for radio/checkbox selection; omit for free text. Set 'multiSelect: true' for checkboxes. Form has one Submit + Cancel at the bottom. Example: { questions: [{header:'Language',question:'What language?',options:[{label:'Python'},{label:'Go'}]},{header:'Framework',question:'What framework?'}] }. RULES: NEVER use 'questions' array for a single question — use mode A or B. ALWAYS use 'choices' when the question has predefined options. ALWAYS populate 'context' with your FULL response (explanations, code, analysis). The user reads this from FlowCommand's remote UI — without it they see nothing. NOTE: For presenting detailed multi-step plans or proposals that need user sign-off, use plan_review instead.",
