...
export function activate(context: vscode.ExtensionContext) {
  ...
  const provider = new FlowCommandWebviewProvider(
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      FlowCommandWebviewProvider.viewType,
      provider,
    ),
  ...
  const configWatcher = vscode.workspace.onDidChangeConfiguration((e) => {
    if (
      e.affectsConfiguration("flowcommand.instructionInjection") ||
      e.affectsConfiguration("flowcommand.instructionText")
    ) {
  ...
  if (mcpEnabled) {
    ...
    hasExternalMcpClientsAsync()
      ...
      .catch((err) => {
        console.error(
          "[FlowCommand] Failed to check external MCP clients:",
          err,
        );
      });
  }
    ...
    async () => {
      if (mcpServer && !mcpServer.isRunning()) {
        ...
        vscode.window.showInformationMessage(
          "FlowCommand MCP Server is already running",
        );
      }
    },
  );
  ...
  const stopMcpCmd = vscode.commands.registerCommand(
    ...
    async () => {
      if (!mcpServer || !mcpServer.isRunning()) {
        vscode.window.showInformationMessage(
          "FlowCommand MCP Server is not running",
        );
        return;
      }
  ...
  const getMcpStatusCmd = vscode.commands.registerCommand(
  ...
  const clearSessionCmd = vscode.commands.registerCommand(
    "flowcommand.clearCurrentSession",
    async () => {
  ...
  const openPromptsCmd = vscode.commands.registerCommand(
    "flowcommand.openPrompts",
    () => {
  ...
  async function updateRemoteStatusBar() {
    ...
    if (remoteServer?.isRunning()) {
      ...
      const networkUrl =
        info.urls.find((u) => !u.includes("localhost")) || info.urls[0];
      ...
      remoteStatusBarItem.backgroundColor = new vscode.ThemeColor(
  ...
  const toggleRemoteCmd = vscode.commands.registerCommand(
    "flowcommand.toggleRemoteServer",
    async () => {
      ...
      if (remoteServer.isRunning()) {
        ...
        void updateRemoteStatusBar();
        vscode.window.showInformationMessage(
          "FlowCommand Remote Server stopped",
        );
      } else {
        try {
          ...
          vscode.window.showErrorMessage(
            `Failed to start remote server: ${err}`,
          );
        }
      }
    },
  ...
  const startRemoteCmd = vscode.commands.registerCommand(
    "flowcommand.startRemoteServer",
    async () => {
      if (!remoteServer) return;
  ...
  const showRemoteUrlCmd = vscode.commands.registerCommand(
    "flowcommand.showRemoteUrl",
    async () => {
      if (!remoteServer || !remoteServer.isRunning()) {
  ...
  const reinjectInstructionsCmd = vscode.commands.registerCommand(
    "flowcommand.reinjectInstructions",
    async () => {
  ...
  const getInstructionStatusCmd = vscode.commands.registerCommand(
  ...
  context.subscriptions.push(
    ...
    stopRemoteCmd,
    showRemoteUrlCmd,
    reinjectInstructionsCmd,
    getInstructionStatusCmd,
    remoteServer,
  );
}
...
function getStoredInjectionState(): InjectionState | undefined {
  ...
  return extensionContext.workspaceState.get<InjectionState>(
    INJECTION_STATE_KEY,
  );
}
 ...
 */
async function storeInjectionState(
  state: InjectionState | undefined,
): Promise<void> {
 ...
 */
async function checkCopilotInstructionsState(
  expectedContent: string,
): Promise<"correct" | "missing" | "modified" | "corrupted" | "no-file"> {
  ...
  try {
    ...
    const currentSection = content.substring(
      startIdx,
      endIdx + FLOWCOMMAND_SECTION_END.length,
    );
 ...
 */
function checkCodeGenSettingsState(
  expectedContent: string,
): "correct" | "missing" | "modified" {
  const settingsText = `${FLOWCOMMAND_MARKER} ${expectedContent}`;
  const copilotConfig = vscode.workspace.getConfiguration(
    "github.copilot.chat",
  );
  const currentInstructions = copilotConfig.get<
    Array<{ text?: string; file?: string }>
...
/**
 * Get a stable hash of the FlowCommand section currently in copilot-instructions.md.
 * Used to suppress repeated prompts when user intentionally keeps a modified section.
 */
async function getCopilotInstructionsSectionHash(): Promise<
  string | undefined
> {
  const workspaceFolders = vscode.workspace.workspaceFolders;
...
/**
 * Get a stable hash of the FlowCommand codeGeneration.instructions entry.
 * Used to suppress repeated prompts when user intentionally keeps a modified entry.
 */
function getCodeGenInstructionEntryHash(): string | undefined {
  const copilotConfig = vscode.workspace.getConfiguration(
    "github.copilot.chat",
  );
  const currentInstructions = copilotConfig.get<
  ...
  if (!existingEntry?.text) return undefined;
 ...
 */
async function handleInstructionInjection(
    ...
    switch (method) {
      case "copilotInstructionsMd": {
        ...
        if (fileState === "correct" && !forceInject) {
          ...
          await storeInjectionState({
            method,
            contentHash,
            timestamp: Date.now(),
          });
          console.log(
            "[FlowCommand] Instructions already correctly injected in copilot-instructions.md",
          );
          return;
        }
        ...
        if (fileState === "corrupted") {
          ...
          if (forceInject) {
            ...
            const sectionHash = await getCopilotInstructionsSectionHash();

            // User already chose to keep this exact modified content; don't prompt repeatedly on restart.
            if (
              sectionHash &&
              storedState?.method === method &&
              storedState?.contentHash === contentHash &&
              storedState?.acceptedModifiedHash === sectionHash
            ) {
              return;
            }
            ...
            if (action === "Re-inject") {
              ...
              await storeInjectionState({
                ...
                contentHash,
                timestamp: Date.now(),
                acceptedModifiedHash: sectionHash,
              });
            }
            return;
          }
      ...
      case "codeGenerationSetting": {
        ...
        if (settingsState === "correct" && !forceInject) {
          ...
          console.log(
            "[FlowCommand] Instructions already correctly injected in codeGeneration settings",
          );
          return;
        }

        if (
          (settingsState === "correct" || settingsState === "modified") &&
          forceInject
        ) {
          // Force re-inject: update the settings entry
          injectIntoCodeGenSettings();
        } else if (settingsState === "modified" && !forceInject) {
          const entryHash = getCodeGenInstructionEntryHash();
          ...
          if (
            ...
            storedState?.method === method &&
            storedState?.contentHash === contentHash &&
            storedState?.acceptedModifiedHash === entryHash
          ) {
            return;
          }
          ...
          if (action === "Re-inject") {
            ...
            await storeInjectionState({
              ...
              contentHash,
              timestamp: Date.now(),
              acceptedModifiedHash: entryHash,
            });
          }
          return;
 ...
 */
async function injectIntoCopilotInstructionsMd(
  ...
  try {
    ...
    if (fileExists && existingContent.includes(FLOWCOMMAND_SECTION_START)) {
      ...
      if (startIdx !== -1 && endIdx !== -1) {
        const currentSection = existingContent.substring(
          startIdx,
          endIdx + FLOWCOMMAND_SECTION_END.length,
        );
        ...
        if (currentSection !== newSection) {
          const updatedContent =
            ...
            newSection +
            existingContent.substring(endIdx + FLOWCOMMAND_SECTION_END.length);
          await vscode.workspace.fs.writeFile(
            filePath,
            Buffer.from(updatedContent, "utf-8"),
          );
          console.log(
            "[FlowCommand] Updated instructions in copilot-instructions.md",
          );
        }
      }
      return;
    ...
    if (fileExists) {
      ...
      console.log(
        "[FlowCommand] Appended instructions to copilot-instructions.md",
      );
    } else {
      ...
      console.log(
        "[FlowCommand] Created copilot-instructions.md with instructions",
      );
    }
  } catch (err) {
    console.error(
      "[FlowCommand] Failed to inject into copilot-instructions.md:",
      err,
    );
  }
}
...
async function removeFromCopilotInstructionsMd(): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      updatedContent === "# Copilot Instructions"
    ) {
      // File would be empty or just the header we created — delete it
      await vscode.workspace.fs.delete(filePath);
      console.log(
        "[FlowCommand] Removed copilot-instructions.md (was only FlowCommand content)",
      );
    } else {
      await vscode.workspace.fs.writeFile(
        filePath,
      ...
      console.log(
        "[FlowCommand] Removed FlowCommand section from copilot-instructions.md",
      );
    }
  } catch {
  ...
  if (existingIndex === -1) {
    ...
    copilotConfig
      ...
      .then(
        ...
        (err: unknown) =>
          console.error(
            "[FlowCommand] Failed to inject settings instructions:",
            err,
          ),
      );
    ...
    copilotConfig
      ...
      .then(
        ...
        (err: unknown) =>
          console.error(
            "[FlowCommand] Failed to update settings instructions:",
            err,
          ),
      );
  }
  ...
  if (filtered.length !== currentInstructions.length) {
    ...
    copilotConfig
      ...
      .then(
        ...
        (err: unknown) =>
          console.error(
            "[FlowCommand] Failed to remove settings instructions:",
            err,
          ),
      );
  }
