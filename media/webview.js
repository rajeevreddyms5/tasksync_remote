...
(function () {
  ...
  function updateQueuePauseUI() {
    // Re-query in case DOM was rebuilt (handles webview restoration)
    queuePauseBtn = document.getElementById("queue-pause-btn");
  ...
  function showPendingToolCall(id, prompt, isApproval, choices, context) {
    ...
    // Focus input for typing - only if auto-focus is enabled
    if (autoFocusPanelEnabled && chatInput) {
      chatInput.focus();
    }
  }
  ...
  /**
   * Show approval modal
   * Recreates buttons and rebinds handlers to handle webview DOM restoration
   */
  function showApprovalModal() {
    // Get or create approval bar (handles webview restoration where DOM was rebuilt)
    approvalModal = document.getElementById("approval-bar");
    if (!approvalModal) {
      // Create approval bar that appears at the top of input-wrapper (inside the border)
      approvalModal = document.createElement("div");
      approvalModal.className = "approval-bar";
      approvalModal.id = "approval-bar";
      approvalModal.setAttribute("role", "toolbar");
      approvalModal.setAttribute("aria-label", "Quick approval options");
    ...
    // Rebuild content and rebind handlers (ensures fresh bindings after DOM restore)
    approvalModal.innerHTML =
      '<span class="approval-label">Waiting on your input..</span>' +
      '<div class="approval-buttons">' +
      '<button class="approval-btn approval-reject-btn" aria-label="Reject and provide custom response">No</button>' +
      '<button class="approval-btn approval-accept-btn" aria-label="Yes and continue">Yes</button>' +
      "</div>";
    ...
    // respecting the autoFocusPanelEnabled setting
  }
  ...
  function hideApprovalModal() {
    // Re-query by ID to handle webview DOM restoration
